...
Public Class wndPreferences
    ...
    Private Sub wndPreferences_Loaded(sender As Object, e As RoutedEventArgs) Handles Me.Loaded
        ...
        cmboSIPProtocol.ItemsSource = protocols
        dpSIPState.DataContext = sip
    End Sub
    ...
    Private Sub lvAttributesDefault_Drop(sender As Object, e As DragEventArgs) Handles lvAttributesDefault.Drop
        If e.Data.GetDataPresent("clsAttribute") And sourceobject Is lvAttributesForSearch Then
            ...
            If preferences.AttributesForSearch.Contains(obj) Then
                If preferences.AttributesForSearch.Count <= 1 Then IMsgBox("Необходим хотя бы один атрибут", "Атрибуты для поиска", vbOKOnly, vbExclamation) : Exit Sub
    ...
    Private Sub chbStartWithWindows_Checked(sender As Object, e As RoutedEventArgs) Handles chbStartWithWindows.Checked, chbStartWithWindows.Unchecked, chbStartWithWindowsMinimized.Checked, chbStartWithWindowsMinimized.Unchecked
        If chbStartWithWindows.IsChecked Then
            If chbStartWithWindowsMinimized.IsChecked Then
                My.Computer.Registry.CurrentUser.OpenSubKey("SOFTWARE\Microsoft\Windows\CurrentVersion\Run", True).SetValue("ADViewer", """" & System.Reflection.Assembly.GetExecutingAssembly().Location & """ -minimized") 'My.Application.Info.AssemblyName)
            Else
                My.Computer.Registry.CurrentUser.OpenSubKey("SOFTWARE\Microsoft\Windows\CurrentVersion\Run", True).SetValue("ADViewer", System.Reflection.Assembly.GetExecutingAssembly().Location) 'My.Application.Info.AssemblyName)
            End If
        Else
            My.Computer.Registry.CurrentUser.OpenSubKey("SOFTWARE\Microsoft\Windows\CurrentVersion\Run", True).DeleteValue("ADViewer")
        End If
    End Sub
    ...
    Private Sub tbSipPassword_LostFocus(sender As Object, e As RoutedEventArgs) Handles tbSipPassword.LostFocus
        preferences.SipPassword = tbSipPassword.Password
    End Sub

    Private Sub btnSipConnect_Click(sender As Object, e As RoutedEventArgs) Handles btnSipConnect.Click
        If sip.StackState = "Started" Then
            sip.Unregister()
        Else
            sip.Register()
        End If
