...
Public Class ctlGroupMember
    ...
    Private Shared Sub CurrentObjectPropertyChanged(d As DependencyObject, e As DependencyPropertyChangedEventArgs)
        ...
        With instance
            ...
            ._currentobject.member.ToList.ForEach(Sub(x As clsDirectoryObject) .lvSelectedObjects.Items.Add(x))
        ...
        If e.Data.GetDataPresent("dolist") And sender IsNot sourceobject Then
            ...
            For Each obj In dragged
                If sender Is lvSelectedObjects Then ' adding member
                    If obj.Domain IsNot _currentobject.Domain Then IMsgBox("Из другого домена нельзя же!", "Ну тычоваще :(", vbOKOnly, vbExclamation) : Exit Sub
                    AddMember(obj)
                Else
                    RemoveMember(obj)
                End If
    ...
    Private Sub AddMember([object] As clsDirectoryObject)
        ...
        Catch ex As Exception
            ...
            If _currentobject IsNot Nothing And [object] IsNot Nothing AndAlso _currentobject.SchemaClassName = "group" And [object].SchemaClassName = "group" Then ShowWrongMemberMessage()
        End Try
    End Sub
