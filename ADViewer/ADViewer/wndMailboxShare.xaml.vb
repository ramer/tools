...
Public Class wndMailboxShare
    ...
    Private Async Sub tbSearchPattern_KeyDown(sender As Object, e As KeyEventArgs) Handles tbSearchPattern.KeyDown
        If e.Key = Key.Enter Then
            Await searcherusers.BasicSearchAsync(users, CType(sender, TextBox).Text, currentuser.Domain,, True, False, True, False)
        End If
    End Sub
        ...
        If e.Data.GetDataPresent("clsDirectoryObject") And sender IsNot sourceobject Then
            ...
            If sender Is lvSendAs Then
                If Not mailbox.PermissionSendAs.Contains(obj) Then
                    If lvSendAs.Items.Count = 0 AndAlso Not chbSentItemsConfigurationSendAs.IsChecked = True AndAlso IMsgBox("Помещать копии писем в папку ""Отправленные""?", "Действие при отправке", vbYesNo, vbQuestion) = MsgBoxResult.Yes Then
            ...
            ElseIf sender Is lvSendOnBehalf Then
                If Not mailbox.PermissionSendOnBehalf.Contains(obj) Then
                    If lvSendAs.Items.Count = 0 AndAlso Not chbSentItemsConfigurationSendOnBehalf.IsChecked = True AndAlso IMsgBox("Помещать копии писем в папку ""Отправленные""?", "Действие при отправке", vbYesNo, vbQuestion) = MsgBoxResult.Yes Then
